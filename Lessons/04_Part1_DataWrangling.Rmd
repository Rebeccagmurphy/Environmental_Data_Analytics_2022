...
library(tidyverse) #Just a collection of packages
#install.packages(lubridate)
library(lubridate) #Just for date functions
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE) #allows summary to work for character entries
...
## Data Wrangling aka cleaning the dataset
...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #if you want to look at surface information (aka rows where depth is at 0)
#again note that [0,9], 0 is row, 9 is column

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) #first argument is dataset, second is what you want to filter
...
#cleaner way to filter the data set
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #same thing as above, considered combination of both
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304)
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #again, cleanest option.... 
