...
NTL.phys.data.Tuesday.Years <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Row numbers are somewhat abritrary.... 
...
NTL.phys.data.mutate <- mutate(NTL.phys.data, month = month(sampledate))
  
NTL.phys.data.mutate.July <- filter(NTL.phys.data.mutate, lakename == "Tuesday Lake", year4 %in% c(1990:1999), month == 7)

# or 
# Exercise 3 <- NTL.phys.data %>%
  # filter(year4 %in% 1990:1990)
...
NTL.phys.data.temperature <- NTL.phys.data.mutate.July %>%
  filter(depth == 0) %>%
  filter(!is.na(temperature_C)) %>% #alternative: dropna(temperature_C)
