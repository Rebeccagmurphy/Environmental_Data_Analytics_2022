...
## review of hypothesis testsing
- to analyze evidence provided by data
- To make decision based on data

Procedure:
- state the hypothese and identify the claim
2. Find the critical values from the appopriate trable
3. Compute the test value
4. Make the dceision to reject or not reject the null hypothesis
...
I.e. just one variable on the right side of the equation. 
For the NTL-LTER dataset, can we predict irradiance (light level) from depth?

```{r}
irradiance.regression <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth) #variable y = irradiance, i.e. explaining irradiance based on depth levels (X)
...
2. **Normal Q-Q.** The points should fall close to the 1:1 line. We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. Compares residuals to a normal distribution. 
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #dividing screens into 4 cells and defining the size of the plot
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #change the plotting window to exclude outliers from view
  geom_point() 
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + #yaxis will be log transformed
  geom_point() 
...
## Correlation Plots
We can also make exploratory plots of several continuous data points to determine possible relationships, as well as covariance among explanatory variables. Simplified version of a linear regression - how are variables related to eachother. The skinnier ellipses (the thinness) represents a good relationship - more circular is less of a relationship
