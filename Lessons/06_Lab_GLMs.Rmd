...
Plot2 <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) + #y comes from the data itself
  geom_density(fill = "Yellow") #set year as factor
print(Plot2)

Plot3 <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
  stat_summary(geom = "line", fun.y = max, vjust = -1, size = 4, label= c("a","b", color = 'red')) +
