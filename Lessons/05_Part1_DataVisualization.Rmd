...
getwd()
library(tidyverse)
#install.packages("ggridges") help to see the distribution of a variable over time
library(ggridges)
...
#Same plot but in different way: object to assign ggplots
#Helpful if you want to add several plots to the same page in a report
...
PMplot <- 
  ...
  geom_point() #adds point as the variable
print(PMplot)
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #instead of coloring sites with different colors, you are creating different plots
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #again, creates different graphs
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #because water depth is reversed 
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #creates a trend line e.g. linear model
  geom_abline(aes(slope = 16, intercept = 0)) #creates trend line but you have to know slope and intercept
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? color is outline of shape, fill inside of shape
print(Nutrientplot3)
...
# Frequency polygons
# Using a tidy dataset --- bad example
Nutrientplot6 <-
